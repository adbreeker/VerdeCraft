            ...
            if(reactor.getType().equals(BlockManager.NuclearReactor.getType()) && reactor_chest.getState() instanceof TileState)
            {
                ...
                if(container.has(key, PersistentDataType.INTEGER))
                {
                    ...
                    if(event.getItem() != null)
                    {
                        ...
                        {
                            if(event.getItem().getItemMeta().equals(ItemManager.UraniumCell.getItemMeta()))
                            {
                                if(reactor_fuel < 15360)
                                {
                                    reactor_fuel += 4800;
                                    if(reactor_fuel > 19200)
                                    {
                                        reactor_fuel = 19200;
                                    }
                                    ...
                                    event.getPlayer().sendMessage("Reactor fuel increased to §2" + (reactor_fuel/192) + "%§f, all cores are operational");
                                }
                                else
                                {
                                    event.getPlayer().sendMessage("Reactor fuel §2" + (reactor_fuel/192) + "%§f Refueling available after decaying under §480%");
                                }
                                return;
                            }
                        }
